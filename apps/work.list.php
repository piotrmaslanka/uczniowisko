<?php
	...
	$tpl['fk_overmode'] = $entered_cat->fk_overmode;
	
	if ($argv[3]=='o')
		$res = mysql_query("SELECT COUNT(id) AS cnt FROM work WHERE (mode>0) AND (fk_category IN (SELECT id FROM category WHERE fk_overmode=".$entered_cat->fk_overmode."))");
	else
		$res = mysql_query("SELECT COUNT(id) AS cnt FROM work WHERE (mode>0) AND ((fk_category=$entered_cat->id) OR (fk_category IN (SELECT id FROM category WHERE fk_category=$entered_cat->id)))");
		
	$row = mysql_fetch_array($res);	
	$pages_max = ceil($row['cnt'] / $cfg['works_per_page']);
		/* Get works list */
	if ($argv[3]=='o')
		$res = mysql_query("SELECT * FROM work WHERE (mode>0) AND (fk_category IN (SELECT id FROM category WHERE fk_overmode=".$entered_cat->fk_overmode.")) LIMIT ".(($page-1)*$cfg['works_per_page']).", ".$cfg['works_per_page']);
	else
		$res = mysql_query("SELECT * FROM work WHERE (mode>0) AND ((fk_category=$entered_cat->id) OR (fk_category IN (SELECT id FROM category WHERE fk_category=$entered_cat->id))) LIMIT ".(($page-1)*$cfg['works_per_page']).", ".$cfg['works_per_page']);
